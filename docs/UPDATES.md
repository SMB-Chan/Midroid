...
When all four are available, CI signs the debug APK with that stable key and publishes an additional `Midroid-update-apk` artifact. The normal `Midroid-ci-debug-apk` artifact is still produced for verification.
...
Do not uninstall the stable-track app during ordinary updates. Install the newer APK directly over the existing package.
